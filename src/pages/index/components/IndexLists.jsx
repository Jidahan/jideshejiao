...
import { collectionUser } from "../service";
import "../index.less";
    ...
    Taro.getStorage({
      ...
      complete: (res) => {
        if (res.errMsg === "getStorage:ok") {
          ...
          collectionUser({ otherUserId: info.userId, userId: Number(res.data) })
            .then((data) => {
              if (data.data.status === 200) {
                ...
                Taro.eventCenter.trigger("goLikeUserIsRefresh", {
                  status: true,
                  id: info.userId,
                  nowDisabledUser: info.collectionIs,
                });
              } else {
                Toast.remove(key);
                Toast.fail({
          ...
          <Card.Body style={{ padding: 10 }}>
            <View className="cardBody" onClick={() => cardGoUserInfo(item)}>
                ...
                <View className="cardCenterContent">
                  <View className="row">
                    ...
                    {item.gender === 2 ? (
                      ...
                      <View className="row" style={{ marginLeft: 5 }}>
                        <Image style="width: 18px;height: 18px" src={manImg} />
                      </View>
                    )}
                  </View>
                  <View
                  ...
                  >
                    <View className="row">
                      ...
                      <Text style={{ color: "#d4237a", marginLeft: 2 }}>
                        {item.hotNum}
                      </Text>
                    </View>
                    <View className="row cardContentBottomCenter">
                      <Image style={styles.adStyle} src={adSumImg} />
                      <Text style={{ color: "#FA8100" }}>
                        {item.individualValues}
                      </Text>
                    </View>
                  </View>
                </View>
              ...
              <View className="row cardRightTop">
                <Text style={{ color: "#121212", marginRight: 5 }}>
                  {item.onlineState === 1 ? "在线" : "离线"}
                </Text>
                <View
                  className={`onlineOrNoOnlineStyle ${
                    item.onlineState === 1 ? "online" : "noOnline"
                  }`}
                ></View>
              </View>

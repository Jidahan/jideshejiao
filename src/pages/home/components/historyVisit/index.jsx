...
import { findHistoricalVisitors } from "./service";
...
class Historyvisit extends PureComponent {
    ...
    Taro.getStorage({
      ...
      complete: (res) => {
        if (res.errMsg === "getStorage:ok") {
          ...
          findHistoricalVisitors({ pageNumber, pageSize, userId: res.data })
            .then((data) => {
  ...
  loadData(refresh) {
    if (refresh) {
      ...
      let dataArray = [];
      findHistoricalVisitors({ pageNumber: page, pageSize, userId })
        .then((data) => {
          if (data.statusCode === 200) {
      ...
      this.setState(
        ...
        () => {
          const { pageNumber, pageSize, userId } = this.state;
          findHistoricalVisitors({ pageNumber, pageSize, userId })
            .then((data) => {
              if (data.statusCode === 200) {
                this.setState(
                  { dataArray: data.data.data, isLoading: false },
                  () => {
                    this.loadData(true);
                  }
                );

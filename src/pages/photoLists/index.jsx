...
class Photolists extends Component {
  ...
  componentDidMount() {
    const { route:{params:{userId, parent}} } = this.props
    if(parent === 'home') {this.setState({ isShowDel: true })} else { this.setState({ isShowDel: false }) }
    getUserPhotos({
      pageNumber: 1,
      pageSize: 500,
      userId
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={{ height: 75, backgroundColor: 'white', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 35}}>
          <View onClick={this.goBack}>
            <Image src={gobackImg} style={{ width: 20, height: 20, marginLeft: 20 }} />
          </View>
          <Text style={{ fontSize: 18, fontWeight: 'bold' }}>相册</Text>
          {
            isShowDel ?
              ...
              <View onClick={this.goDel}>
                <Image src={deleteImg} style={{ width: 20, height: 20, marginRight: 20 }} />
              </View>
              :
              <Text style={{ marginRight: 11, fontSize: 14 }} onClick={this.goNoDel}>取消</Text>
            :
            ...
            <Flex direction='row' justify='between' wrap='wrap'>
              {
                cardList.map(reward => {
                  if(reward.type === 1){
                    return (
                      <View style={{width: 100, height: 100, marginBottom: 10}} key={reward.id} onClick={() => this.myPhotosClick(reward.url)}>
                        <ImageBackground source={{uri: reward.url}} style={{width: '100%', height: '100%', zIndex: 1}} imageStyle={{borderRadius: 10}}>
                          {isDel?
                    ...
                    return (
                      <View style={{width: 100, height: 100, marginBottom: 10}} key={reward.id} onClick={() => this.myVideoClick(`videocc${reward.id}`)}>
                        <Video
              ...
              {
                cardList.length % 2 === 0 ?
                <View style={{width: 100, height: 100, marginBottom: 10}}></View>
                :
                <View style={{width: 100, height: 100, marginBottom: 10}}></View>
              }
            </Flex>

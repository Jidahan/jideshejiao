...
import "./index.less";
import { myFollowList } from "./service";
...
class Focus extends PureComponent {
    ...
    Taro.getStorage({
      ...
      complete: (res) => {
        if (res.errMsg === "getStorage:ok") {
          ...
          myFollowList({ pageNumber, pageSize, userId: res.data })
            .then((data) => {
  ...
  loadData(refresh) {
    if (refresh) {
      ...
      let dataArray = [];
      myFollowList({ pageNumber: page, pageSize, userId })
        .then((data) => {
          if (data.statusCode === 200) {
      ...
      this.setState(
        ...
        () => {
          const { pageNumber, pageSize, userId } = this.state;
          myFollowList({ pageNumber, pageSize, userId })
            .then((data) => {
              if (data.statusCode === 200) {
                this.setState(
